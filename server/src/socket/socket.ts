...
const userSocketIdMap: Record<string, string> = {}; // {userId: socketId}
...
io.on("connection", (socket) => {
  ...
  if (typeof userId === "string") userSocketIdMap[userId] = socket.id;
  ...
  socket.on("disconnect", () => {
    ...
    if (typeof userId === "string") delete userSocketIdMap[userId];
    socket.emit("getOnlineUser", Object.keys(userSocketIdMap));
  });
});
